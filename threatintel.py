...
import re, sys, json, os, socket, argparse, urllib.request, urllib.error
...
def print_report(result):
    ...
    if result.target_type in ("ip","domain"):
        ...
        divider("REPUTATION")
        if result.sources.get("vt_queried"):
        ...
        if result.sources.get("abuse_queried"):
            ab_color = red if result.abuse_score>50 else orange if result.abuse_score>10 else green
            row("AbuseIPDB Score", f"{result.abuse_score}/100", ab_color)
        else: row("AbuseIPDB", "No key — use --abuse-key", muted)
        if result.sources.get("otx_queried"):
            row("OTX Pulses", str(result.otx_pulses), red if result.otx_pulses>0 else green)
...
CONFIG_FILE = os.path.expanduser("~/.threatintel.conf")
...
def run_setup():
    print_banner()
    print(f"{cyan('─'*54)}")
    print(f"  {white('First-Time Setup — API Keys')}")
    print(f"  {muted('Keys are saved to ~/.threatintel.conf')}")
    print(f"  {muted('Press Enter to skip any key')}")
    print(f"{cyan('─'*54)}\n")

    cfg = load_config()

    print(f"  {cyan('VirusTotal')} → virustotal.com → Profile → API Key")
    vt = input(f"  {muted('VT Key')} [{muted(cfg.get('vt_key','not set'))}]: ").strip()
    if vt: cfg["vt_key"] = vt

    print(f"\n  {cyan('AbuseIPDB')} → abuseipdb.com → Account → API")
    abuse = input(f"  {muted('AbuseIPDB Key')} [{muted(cfg.get('abuse_key','not set'))}]: ").strip()
    if abuse: cfg["abuse_key"] = abuse

    print(f"\n  {cyan('AlienVault OTX')} → otx.alienvault.com → Settings → API Key")
    otx = input(f"  {muted('OTX Key')} [{muted(cfg.get('otx_key','not set'))}]: ").strip()
    if otx: cfg["otx_key"] = otx

    save_config(cfg)
    print(f"\n  {green('✓')} Keys saved to {cyan(CONFIG_FILE)}")
    print(f"  {muted('Now just run:')} {white('threatintel 8.8.8.8')}\n")
    sys.exit(0)
...
Examples:
  ...
  threatintel 1.2.3.4 --output report.html --json results.json
...
First time setup:
  ...
  threatintel --setup
    ...
    group.add_argument("--setup", action="store_true", help="Save API keys to config (run once)")
    parser.add_argument("--vt-key",    metavar="KEY", help="VirusTotal API key (overrides config)")
    parser.add_argument("--abuse-key", metavar="KEY", help="AbuseIPDB API key (overrides config)")
    parser.add_argument("--otx-key",   metavar="KEY", help="AlienVault OTX API key (overrides config)")
    ...
    # Load saved config, override with CLI flags if provided
    cfg = load_config()
    vt_key    = getattr(args, "vt_key",    None) or cfg.get("vt_key")
    abuse_key = getattr(args, "abuse_key", None) or cfg.get("abuse_key")
    otx_key   = getattr(args, "otx_key",   None) or cfg.get("otx_key")
    ...
    if vt_key:
        if result.target_type=="ip": fetch_vt_ip(result, vt_key)
        elif result.target_type=="domain": fetch_vt_domain(result, vt_key)
        else: fetch_vt_hash(result, vt_key)
        result.sources["vt_queried"] = True
        print(f"  {green('✓')} VirusTotal")
    if abuse_key and result.ip:
        fetch_abuse(result, abuse_key)
        result.sources["abuse_queried"] = True
        print(f"  {green('✓')} AbuseIPDB")
    if otx_key:
        fetch_otx(result, otx_key)
        result.sources["otx_queried"] = True
        print(f"  {green('✓')} OTX")
